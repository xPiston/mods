...
public class Smaz
{
    private static Logger       log                = Logger.getLogger("Smaz");
     ...
     * @return byte array
     */
    public byte[] compress(final String inString)
    {
        this.confirmOnlyAscii(inString);

        final StringBuilder verb = new StringBuilder();
        final ByteArrayOutputStream output = new ByteArrayOutputStream();

        final CharBuffer charBuffer = CharBuffer.wrap(inString);
        int inlen;
        ...
        while ((inlen = charBuffer.remaining()) > 0)
        {
            ...
            for (; j > 0; j--)
            {
                CharBuffer slot;
                if (j == 1)
                    slot = CharBuffer.wrap(Smaz.CODEBOOK[h1 % 241]);
                else if (j == 2)
                    slot = CharBuffer.wrap(Smaz.CODEBOOK[h2 % 241]);
                else
                    slot = CharBuffer.wrap(Smaz.CODEBOOK[h3 % 241]);
                ...
                while (slotLength > 0 && slotEndIndex <= slotLength)
                            ...
                            .toString().equals(charBuffer.subSequence(0, j).toString()))
                    {
                        ...
                        if (verb.length() > 0)
                        {
                            ...
                            this.outputVerb(output, verb.toString());
                            verb.setLength(0);
                        }
     ...
     * @param str
     */
    private void outputVerb(final ByteArrayOutputStream baos, final String str)
    {
        if (str.length() == 1)
        {
            baos.write(254);
        ...
        else
        {
            ...
            {
                Smaz.log.log(Level.SEVERE, "Error outputting verbatim data", e);
            }
        }
    }

    private void confirmOnlyAscii(final String input)
    {
     ...
     */
    public String decompress(final byte[] strBytes)
    {
        ...
        for (int i = 0; i < strBytes.length; i++)
        {
            ...
            else
            {
                ...
                out.append(Smaz.REVERSE_CODEBOOK[loc]);
            }
        }
        return out.toString();
    }
